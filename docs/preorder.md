...
| ฟิลด์ | ความหมาย |
|---|---|
| `reason` | `restock` = สินค้ากำลังจะเข้า (ขาดสต็อก) · `price_increase` = สินค้าขาดสต็อกและผู้ผลิตแจ้งปรับราคาขึ้น (มติ 9 ก.ย. 69 ใช้พรีออเดอร์แทนวิธี "เอ่งชิ้ว" เดิมที่หยุดขายรอปรับราคาแล้วค่อยเปิดขาย ซึ่งสร้างปัญหา) ทั้งสองกรณีสินค้าต้องขาดสต็อก หน้าลูกค้าแยกเป็น 2 แท็บตาม reason · หลังบ้านเตือนถ้าสินค้ายังมีสต็อก |
...
- ผลล่าสุด: local **94/94** (10 ก.ย. 2569 commit db5c58e บน DB dump ล่าสุด + notification-service รันในเครื่อง, ตั้ง `PRO_CODE_NO_L1=03091086` และ `SALES_TOKEN` ของ E2ESALES) `docs/e2e/preorder-local-1789059103027.md` · ก่อนหน้า 79/79 (9b9bda9), 78/78 (f0a4233, 1422c4f) · ก่อนหน้า 71/71 (1757f26), 43/43, 23/23, 22/22 · deployed: ยังไม่ได้รัน
...
- [x] แก้ถ้อยคำแท็บ "สินค้าจะมีการปรับราคา" ตามที่มาของมติ (เอ่งชิ้ว = วิธีเดิมที่หยุดขายรอปรับราคา → เปลี่ยนเป็นพรีออเดอร์เพื่อความโปร่งใส): การ์ดแสดงราคาใหม่/วันมีผล และบอกว่าราคาที่เรียกเก็บเป็นราคา ณ วันยืนยันคำสั่งซื้อ ไม่สัญญาราคาเดิม · หลังบ้านเตือนเมื่อสินค้ายังมีสต็อก · คำถามที่เหลือ (ล็อคราคาไหม, วันปิดรับ/เพดาน) อยู่ใน [UAT & Demo Guide](https://nitipongjin-13063.atlassian.net/wiki/spaces/R/pages/202473517) ถามตอน UAT
