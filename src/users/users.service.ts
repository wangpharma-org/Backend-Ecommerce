...
export class UsersService {
  ...
  > {
    try {
      ...
      if (diffInDays < 45) {
        return `Most recently used within 45 days`;
      } else if (diffInDays >= 45 && diffInDays < 60) {
        console.log(
          'Latest purchase within 45 days:',
          user.latest_purchase,
          diffInDays,
        );
        return `Latest purchase within 45 days`;
      } else if (diffInDays >= 60) {
        console.log('More than 60 days ago', user.latest_purchase, diffInDays);
        return {
          message: 'More than 60 days ago',
          employee: user.employee ?? undefined,
        };
      }
      throw new Error('No recent purchase within 45 days');
    } catch (error) {
